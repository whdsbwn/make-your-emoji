    ...
    for tracker in accelerator.trackers:
        ...
        if tracker.name == "wandb":
            tracker.log(
                {
                    "validation": [
                        wandb.Image(image, caption=f"global step: {global_step} / {i}: {args.validation_prompt}") for i, image in enumerate(images)
                    ]
                }
            )
...
def main(args, progress=None):
    ...
    # run_name = f"{model_name}_{args.learning_rate}_{args.max_train_steps}"
    # wandb.init(
    #         project="dreambooth",
    #         name=run_name,
    #         config=vars(args),
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if progress is not None:
                    progress((global_step + 1) / args.max_train_steps, desc=f"Training Progress")
