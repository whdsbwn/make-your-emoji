...
from diffusers import AutoencoderKLTemporalDecoder, EulerDiscreteScheduler, StableDiffusionPipeline
...
from train_dreambooth import parse_args, main

output_dir_video = "./outputs/videos"
output_dir_frame = "./outputs/frames"
output_path = "./ckpts/dreambooth"
...
############################################ Dreambooth ############################################
def run_dreambooth(steps, prompt):

    pipe = StableDiffusionPipeline.from_pretrained(output_path, torch_dtype=torch.float16).to("cuda")

    image = pipe(prompt, num_inference_steps=steps, guidance_scale=7.5).images[0]

    return image

def save_images_to_folder(uploaded_files, folder_name="./instance_data"):
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    saved_files = []
    for uploaded_file in uploaded_files:
        source_path = uploaded_file.name
        destination_path = os.path.join(folder_name, os.path.basename(source_path))
...
def delete_folder(folder_name="./instance_data"):
...
def finetune_dreambooth(images, class_name):
    ...
    input_args = [
        ...
        "--instance_data_dir", "/workspace/MOFA-Video-Traj/instance_data",
    ...
    message = main(args, progress=gr.Progress())
...
with gr.Blocks() as demo:
    gr.Markdown("""<h1 align="center">Create your own emoji!</h1><br>""")

    gr.Markdown(
        """
        Kindly follow these below instructions:
        <br>
        1. Upoad 3 ~ 5 of your own subject images and write the class name of the subject. (e.g. dog, cat, etc.) <br>
        2. Write your desired text in the prompt box. <br>
            e.g. A photo of [sks] dog swimming in the ocean with waves. <br>
        3. Draw trajectories on the output image: <br>
            3.1. Click "Add Trajectory" first, then select points on the "Add Trajectory Here" image. The first click sets the starting point. Click multiple points to create a non-linear trajectory. To add a new trajectory, click "Add Trajectory" again and select points on the image. Avoid clicking the "Add Trajectory" button multiple times without clicking points in the image to add the trajectory, as this can lead to errors. <br>
            3.2. After adding each trajectory, an optical flow image will be displayed automatically. Use it as a reference to adjust the trajectory for desired effects (e.g., area, intensity). <br>
            3.3. To delete the latest trajectory, click "Delete Last Trajectory." <br>
            3.4. Choose the Control Scale in the bar. This determines the control intensity. Setting it to 0 means no control (pure generation result of SVD itself), while setting it to 1 results in the strongest control (which will not lead to good results in most cases because of twisting artifacts). A preset value of 0.6 is recommended for most cases. <br>
            3.5. To use the motion brush for restraining the control area of the trajectory, click to add masks on the "Add Motion Brush Here" image. The motion brush restricts the optical flow area derived from the trajectory whose starting point is within the motion brush. The displayed optical flow image will change correspondingly. Adjust the motion brush radius using the "Motion Brush Radius" bar. <br>
    ...
    def preprocess_image(image):
        ...
        if isinstance(image, np.ndarray):
            ...
            raise ValueError("Invalid image type. Please upload an image file.")
    ...
    with gr.Row():
        ...
        class_name = gr.Textbox(label="Class name", placeholder="Enter your subject's class name.")
        train_dreambooth_button = gr.Button(value="Train DreamBooth")
        status = gr.Textbox(label="Status")

    with gr.Row():
        with gr.Column(scale=6):
            dreambooth_steps = gr.Slider(label="Inference Steps", minimum=1, maximum=100, step=1, value=50)
            dreambooth_prompt = gr.Textbox(label="Prompt", placeholder="Enter your text prompt.")
            run_dreambooth_button = gr.Button(value="Run DreamBooth")

        with gr.Column(scale=6):
            dreambooth_output = gr.Image(label="DreamBooth Output", interactive=False)
    ...
    # Set up callbacks (function, inputs, outputs)

    train_dreambooth_button.click(finetune_dreambooth, [image_input, class_name], status)

    run_dreambooth_button.click(run_dreambooth, [dreambooth_steps, dreambooth_prompt], [dreambooth_output])

    dreambooth_output.change(preprocess_image, dreambooth_output, [input_image, input_image_mask, first_frame_path, tracking_points, motion_brush_points, motion_brush_mask, motion_brush_viz])
